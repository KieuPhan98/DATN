...
public class ExportExcelLoiNhuan {
    ...
    private void writeHeaderLine(String name){
        ...
        createCell(row, 3, "Tổng Số Lượng Xuất", style);
        createCell(row, 4, "Đơn Giá Xuất TB", style);
        createCell(row, 5, "Đơn Giá Nhập TB", style);
        createCell(row, 6, "Lợi Nhuận", style);
        
        Row row0 = sheet.createRow(0);
        createCell(row0, 0, "SHOP MỸ PHẨM YUMI", style);
        
        Row row2 = sheet.createRow(2);
        createCell(row2, 3, "THỐNG KÊ LỢI NHUẬN", style);
    ...
    private void writeDataLines() {
        ...
        for (Object[] item : listLN) {
            ...
            createCell(row, columnCount++, item[0], style);
